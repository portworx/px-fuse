...
static int pxtgt_open(struct block_device *bdev, fmode_t mode) {
        ...
        trace_pxtgt_open(pxtgt_dev->dev_id, pxtgt_dev->major, pxtgt_dev->minor,
                         mode, err);
        return err;
}
...
static void pxtgt_release(struct gendisk *disk, fmode_t mode) {
        ...
        trace_pxtgt_release(pxtgt_dev->dev_id, pxtgt_dev->major,
                            pxtgt_dev->minor, mode);
...
long pxtgt_ioctl_dump_fc_info(void) {
        ...
        for (i = 0; i < pxtgt_num_contexts; ++i) {
                ...
                printk(KERN_INFO "%s: pxtgt_ctx: %s ndevices: %lu", __func__,
                       ctx->name, ctx->num_devices);
        }
        return 0;
}
...
static long pxtgt_ioctl_init(struct file *file, void __user *argp) {
        ...
        iov_iter_init(&iter, WRITE, &iov, 1,
                      sizeof(struct pxtgt_ioctl_init_args));
...
static void pxtgt_process_fileio(struct work_struct *wi) {
        struct pxtgt_io_tracker *head =
            container_of(wi, struct pxtgt_io_tracker, wi);
...
#endif
{
    ...
    (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 12, 0) &&                         \
     defined(bvec_iter_sectors))
        ...
        generic_end_io_acct(bio_data_dir(bio), &pxtgt_dev->disk->part0,
                            head->start);
#else
...
#endif
                printk_ratelimited(
                    KERN_ERR "pxtgt basic sanity fail, pxtgt_device %px "
                             "(%llu), rw %#x\n",
...
ssize_t pxtgt_add(struct pxtgt_context *ctx, struct pxtgt_add_out *add) {
        ...
        new_minor =
            ida_simple_get(&pxtgt_minor_ida, 1, 1 << MINORBITS, GFP_KERNEL);
        if (new_minor < 0) {
                err = new_minor;
                goto out_module;
        }
        ...
        pxtgt_dev->wq =
            alloc_workqueue("pxtgt%llu", WQ_SYSFS | WQ_UNBOUND | WQ_HIGHPRI, 0,
                            pxtgt_dev->dev_id);
        if (!pxtgt_dev->wq) {
                err = -ENOMEM;
                goto out_id;
        ...
        pxtgt_dev->fp =
            filp_open(add->source, O_DIRECT | O_RDWR | O_LARGEFILE, 0600);
        ...
        printk(KERN_INFO "Device %llu added %px with source %s blockio %d\n",
               pxtgt_dev->dev_id, pxtgt_dev, pxtgt_dev->source,
               pxtgt_dev->block_io);
...
ssize_t pxtgt_ioc_update_size(struct pxtgt_context *ctx,
        ...
        spin_lock(&ctx->lock);
        list_for_each_entry(pxtgt_dev, &ctx->list, node) {
                if ((pxtgt_dev->dev_id == update_size->dev_id) &&
                    !pxtgt_dev->removing) {
                        spin_lock(&pxtgt_dev->lock);
                        found = true;
                        break;
...
void __strip_nl(const char *src, char *dst, int maxlen) {
        ...
        if (len >= maxlen) {
                // to accomodate null
                printk(KERN_WARNING "stripping newline output buffer "
...
static ssize_t pxtgt_cachealloc_set(struct device *dev,
        ...
        mc = pxmgr_cache_alloc(pxtgt_dev->dev_id, pxtgt_dev->size,
                               PXREALM_SMALL, CMODE_PASSTHROUGH, 0, pxtgt_dev);
...
static struct pxtgt_context *dev_to_pxctx(struct device *dev) {
        ...
        struct pxtgt_context *ctx =
            container_of(mdev, struct pxtgt_context, miscdev);
...
static ssize_t pxctx_attached_show(struct device *dev,
        ...
        list_for_each_entry(pxtgt_dev_itr, &ctx->list, node) {
                count +=
                    snprintf(cp, PAGE_SIZE - count, "dev: %llu, source %s\n",
...
int pxtgt_init(void) {
        ...
        for (i = 0; i < pxtgt_num_contexts; ++i) {
                ...
                if (err) {
                        printk(KERN_ERR
                               "pxtgt: failed to initialize connection\n");
                        goto out_fuse;
                }
                ...
                if (err) {
                        printk(KERN_ERR
                               "pxtgt: failed to register dev %s %d: %d\n",
    ...
    (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 12, 0) &&                         \
     defined(bvec_iter_sectors))
        if (bioset_init(&pxtgt_bio_set, PXTGT_MIN_POOL_PAGES,
                ...
                printk(KERN_ERR
                       "pxtgt: failed to initialize bioset_init: -ENOMEM\n");
                goto out_blkdev;
        }
        ppxtgt_bio_set = &pxtgt_bio_set;
#else
        ppxtgt_bio_set = BIOSET_CREATE(
            PXTGT_MIN_POOL_PAGES, offsetof(struct pxtgt_io_tracker, clone));
#endif
