...
static int pxtgt_open(struct block_device *bdev, fmode_t mode) {
  ...
  if (!err) (void)get_device(&pxtgt_dev->dev);
  trace_pxtgt_open(pxtgt_dev->dev_id, pxtgt_dev->major, pxtgt_dev->minor, mode,
                   err);
  return err;
}
...
static void pxtgt_release(struct gendisk *disk, fmode_t mode) {
  ...
  trace_pxtgt_release(pxtgt_dev->dev_id, pxtgt_dev->major, pxtgt_dev->minor,
                      mode);
...
long pxtgt_ioctl_dump_fc_info(void) {
  ...
  for (i = 0; i < pxtgt_num_contexts; ++i) {
    ...
    printk(KERN_INFO "%s: pxtgt_ctx: %s ndevices: %lu", __func__, ctx->name,
           ctx->num_devices);
  }
  return 0;
}
...
static long pxtgt_ioctl_get_version(void __user *argp) {
  ...
  if (argp) {
    ...
    if (copy_to_user(argp + offsetof(struct pxtgt_ioctl_version_args, piv_len),
                     &ver_len, sizeof(ver_len))) {
      return -EFAULT;
    }
    if (copy_to_user(argp + offsetof(struct pxtgt_ioctl_version_args, piv_data),
                     ver_data, ver_len)) {
      return -EFAULT;
    }
  }
...
static long pxtgt_ioctl_init(struct file *file, void __user *argp) {
  struct pxtgt_context *ctx =
      container_of(file->f_op, struct pxtgt_context, fops);
...
static long pxtgt_control_ioctl(struct file *file, unsigned int cmd,
                                unsigned long arg) {
  switch (cmd) {
...
static void pxtgt_compute_checksum(struct pxtgt_device *pxtgt_dev,
  ...
  // compute checksum for each 4K block and write a journal entry with
  // checksum
}
...
static void pxtgt_preprocess_write(struct work_struct *wi) {
  ...
  BUG_ON(head->magic != PXTGT_IOT_MAGIC);
  BUG_ON(pxtgt_dev->magic != PXTGT_DEV_MAGIC);
  // XXX need to first write a journal checksum, data write, journal
  // (commit) step 1: compute checksum
...
{
  struct pxtgt_io_tracker *head =
      container_of(bio, struct pxtgt_io_tracker, clone);
  ...
  pxtgt_io_printk(
      "%s: dev m %d g %lld %s at %ld len %d bytes %d pages "
      "flags 0x%lx\n",
      __func__, pxtgt_dev->minor, pxtgt_dev->dev_id,
      bio_data_dir(bio) == WRITE ? "wr" : "rd", BIO_SECTOR(bio) * SECTOR_SIZE,
      BIO_SIZE(bio), bio->bi_vcnt, bio->bi_flags);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 1)
  bio_end_io_acct(bio, iot->start);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0) || \
    (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 12, 0) &&  \
     defined(bvec_iter_sectors))
  generic_end_io_acct(pxtgt_dev->disk->queue, bio_op(bio),
...
static void pxtgt_process_io(struct pxtgt_io_tracker *head) {
  struct pxtgt_device *pxtgt_dev = head->pxtgt_dev;
...
#endif
{
  ...
  if (!pxtgt_dev || (rw != READ && rw != WRITE)) {
    ...
    printk_ratelimited(KERN_ERR
                       "pxtgt basic sanity fail, pxtgt_device %px "
                       "(%llu), rw %#x\n",
                       pxtgt_dev, (pxtgt_dev ? pxtgt_dev->dev_id : (uint64_t)0),
                       rw);
    bio_io_error(bio);
  ...
  if (cache_enabled(pxtgt_dev)) {
    pxmgr_cache_submit_io(pxtgt_dev->mc, bio);
    return BLK_QC_RETVAL;
  }
  ...
  bio_start_io_acct(bio);
    ...
    (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 12, 0) &&  \
     defined(bvec_iter_sectors))
  generic_start_io_acct(pxtgt_dev->disk->queue, bio_op(bio),
                        REQUEST_GET_SECTORS(bio), &pxtgt_dev->disk->part0);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)
generic_start_io_acct(bio_data_dir(bio), REQUEST_GET_SECTORS(bio),
                      &pxtgt_dev->disk->part0);
#else
_generic_start_io_acct(pxtgt_dev->disk->queue, bio_data_dir(bio),
                       REQUEST_GET_SECTORS(bio), &pxtgt_dev->disk->part0);
#endif
...
static int pxtgt_init_disk(struct pxtgt_device *pxtgt_dev,
  ...
  snprintf(disk->disk_name, sizeof(disk->disk_name), PXTGT_DEV "%llu",
           pxtgt_dev->dev_id);
  ...
  PXTGT_SETUP_CONGESTION_HOOK(q->backing_dev_info, pxtgt_device_congested,
                              pxtgt_dev);
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 7, 0)
...
static void pxtgt_free_disk(struct pxtgt_device *pxtgt_dev) {
  ...
  if (disk->flags & GENHD_FL_UP) {
    del_gendisk(disk);
    if (disk->queue) blk_cleanup_queue(disk->queue);
  }
  put_disk(disk);
}

struct pxtgt_device *find_pxtgt_device(struct pxtgt_context *ctx,
                                       uint64_t dev_id) {
...
ssize_t pxtgt_add(struct pxtgt_context *ctx, struct pxtgt_add_out *add) {
  ...
  if (!pxtgt_dev) goto out_module;

  pxtgt_dev->magic = PXTGT_DEV_MAGIC;
  spin_lock_init(&pxtgt_dev->lock);
  spin_lock_init(&pxtgt_dev->qlock);

  new_minor = ida_simple_get(&pxtgt_minor_ida, 1, 1 << MINORBITS, GFP_KERNEL);
  if (new_minor < 0) {
    err = new_minor;
    goto out_module;
  }
  ...
  pxtgt_dev->wq = alloc_workqueue(
      "pxtgt%llu", WQ_SYSFS | WQ_UNBOUND | WQ_HIGHPRI, 0, pxtgt_dev->dev_id);
  if (!pxtgt_dev->wq) {
    err = -ENOMEM;
    goto out_id;
  }
...
ssize_t pxtgt_remove(struct pxtgt_context *ctx,
  ...
  /* Make sure the req_fn isn't called anymore even if the device hangs
   * around */
...
ssize_t pxtgt_update_size(struct pxtgt_context *fc,
                          struct pxtgt_update_size *update_size) {
  return -EOPNOTSUPP;
}

ssize_t pxtgt_ioc_update_size(struct pxtgt_context *ctx,
                              struct pxtgt_update_size *update_size) {
  bool found = false;
  int err;
  struct pxtgt_device *pxtgt_dev;
...
ssize_t pxtgt_read_init(struct pxtgt_context *ctx, struct iov_iter *iter) {
  ...
  if (copy_to_iter(&pxtgt_init, sizeof(pxtgt_init), iter) !=
      sizeof(pxtgt_init)) {
  ...
  printk(KERN_INFO "%s: pxtgt-control-%d init OK %d devs version %d\n",
         __func__, ctx->id, pxtgt_init.num_devices, pxtgt_init.version);
...
static ssize_t pxtgt_congestion_show(struct device *dev,
  ...
  return sprintf(buf, "congested: %s (%d/%d)\n",
                 ...
                 pxtgt_dev->nr_congestion_on, pxtgt_dev->nr_congestion_off);
}

static ssize_t pxtgt_congestion_set(struct device *dev,
...
void __strip_nl(const char *src, char *dst, int maxlen) {
  ...
  if (len >= maxlen) {
    // to accomodate null
    printk(KERN_WARNING
           "stripping newline output buffer "
           "overflow.. src %d(%s), dst %d\n",
           len, src, maxlen);
    len = maxlen - 1;
  }
...
static void cache_backend_io(struct pxtgt_device *pxtgt_dev, struct bio *bio) {
  struct pxtgt_io_tracker *head;
  ...
  // TODO - handle IO from cache backend
  head = __pxtgt_init_block_head(pxtgt_dev, bio);
  if (!head) {
    BIO_ENDIO(bio, -ENOMEM);
...
static ssize_t pxtgt_cachealloc_set(struct device *dev,
                                    struct device_attribute *attr,
                                    const char *buf, size_t count) {
  struct pxtgt_device *pxtgt_dev = dev_to_pxtgt_dev(dev);
  ...
  mc = pxmgr_cache_alloc(pxtgt_dev->dev_id, pxtgt_dev->size, PXREALM_SMALL,
                         CMODE_PASSTHROUGH, 0, pxtgt_dev, cache_backend_io);
...
static ssize_t pxtgt_cachefree_set(struct device *dev,
                                   struct device_attribute *attr,
                                   const char *buf, size_t count) {
  struct pxtgt_device *pxtgt_dev = dev_to_pxtgt_dev(dev);
  int rc;
...
static DEVICE_ATTR(timeout, S_IRUGO | S_IWUSR, pxtgt_timeout_show,
                   pxtgt_timeout_store);
static DEVICE_ATTR(congested, S_IRUGO | S_IWUSR, pxtgt_congestion_show,
                   pxtgt_congestion_set);
...
static struct attribute *pxtgt_attrs[] = {
    &dev_attr_size.attr,      &dev_attr_major.attr,
    &dev_attr_minor.attr,     &dev_attr_timeout.attr,
    &dev_attr_congested.attr, &dev_attr_calloc.attr,
    &dev_attr_cfree.attr,     NULL};

static struct attribute_group pxtgt_attr_group = {
    .attrs = pxtgt_attrs,
};

static const struct attribute_group *pxtgt_attr_groups[] = {&pxtgt_attr_group,
                                                            NULL};
...
static int pxtgt_sysfs_init(void) {
  ...
  if (err < 0) device_unregister(&pxtgt_root_dev);
...
static int pxtgt_control_open(struct inode *inode, struct file *file) {
  ...
  if (!((uintptr_t)pxtgt_contexts <= (uintptr_t)file->f_op &&
        (uintptr_t)file->f_op <
            (uintptr_t)(pxtgt_contexts + pxtgt_num_contexts))) {
...
static ssize_t pxctx_cache_show(struct device *dev,
                                struct device_attribute *attr, char *buf) {
...
static ssize_t pxctx_cache_set(struct device *dev,
...
static ssize_t pxctx_info_set(struct device *dev, struct device_attribute *attr,
                              const char *buf, size_t count) {
  struct pxtgt_context *ctx = dev_to_pxctx(dev);
  (void)ctx;
...
static ssize_t pxctx_attach_set(struct device *dev,
                                struct device_attribute *attr, const char *buf,
                                size_t count) {
  struct pxtgt_context *ctx = dev_to_pxctx(dev);
  struct pxtgt_add_out add;
...
static ssize_t pxctx_detach_set(struct device *dev,
                                struct device_attribute *attr, const char *buf,
                                size_t count) {
  struct pxtgt_context *ctx = dev_to_pxctx(dev);
  struct pxtgt_remove_out remove;
...
static DEVICE_ATTR(attach, S_IRUGO | S_IWUSR, pxctx_attached_show,
                   pxctx_attach_set);
static DEVICE_ATTR(detach, S_IRUGO | S_IWUSR, pxctx_attached_show,
                   pxctx_detach_set);
...
static void pxtgtctx_set_connected(struct pxtgt_context *ctx, bool enable) {
  ...
  list_for_each(cur, &ctx->list) {
    struct pxtgt_device *pxtgt_dev =
        container_of(cur, struct pxtgt_device, node);
...
int pxtgt_init(void) {
  ...
  pxtgt_contexts =
      kzalloc(sizeof(pxtgt_contexts[0]) * pxtgt_num_contexts, GFP_KERNEL);
  err = -ENOMEM;
  if (!pxtgt_contexts) {
...
#else
  ppxtgt_bio_set = BIOSET_CREATE(PXTGT_MIN_POOL_PAGES,
                                 offsetof(struct pxtgt_io_tracker, clone));
#endif
