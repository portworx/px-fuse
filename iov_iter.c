...
/*
 * Fault in the first iovec of the given __iov_iter, to a maximum length
...
/*
 * Fault in one or more iovecs of the given __iov_iter, to a maximum length of
...
void __iov_iter_init(struct __iov_iter *i, int direction,
...
size_t __iov_iter_zero(size_t bytes, struct __iov_iter *i)
{
	if (unlikely(bytes > i->count))
		bytes = i->count;
...
void __iov_iter_advance(struct __iov_iter *i, size_t size)
{
	iterate_and_advance(i, size, v, 0, 0, 0)
}

/*
 * Return the count of just the current __iov_iter segment.
 */
...
void __iov_iter_kvec(struct __iov_iter *i, int direction,
...
void __iov_iter_bvec(struct __iov_iter *i, int direction,
...
ssize_t __iov_iter_get_pages(struct __iov_iter *i,
...
ssize_t __iov_iter_get_pages_alloc(struct __iov_iter *i,
