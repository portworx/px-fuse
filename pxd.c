...
int pxd_initiate_failover(struct pxd_device *pxd_dev)
{
	...
	// Block fastpath failover if WriteZero→Discard optimization is enabled
	// Fastpath (LVM) doesn't support discard, so WriteZero must remain disabled
	// Allowing failover would cause data corruption (reads after discard return garbage)
	if (pxd_dev->enable_write_zeroes_support) {
		printk(KERN_WARNING "device %llu failover blocked: WriteZero→Discard optimization is enabled\n",
		       pxd_dev->dev_id);
		return -EINVAL;
	}
...
ssize_t pxd_add(struct fuse_conn *fc, struct pxd_add_ext_out *add, bool is_v2)
{
	...
	int new_minor;
	int err;
	struct pxd_add_ext_out_v2 *add_v2 = NULL;

	if (is_v2) {
		add_v2 = (struct pxd_add_ext_out_v2 *)add;
	}
	...
	// Store cluster-level WriteZero support flag (only available in v2)
	if (is_v2 && add_v2) {
		pxd_dev->enable_write_zeroes_support = add_v2->enable_write_zeroes_support;
	} else {
		pxd_dev->enable_write_zeroes_support = 0;  // Default to disabled for v1
	}
