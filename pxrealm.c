...
static void pxrealm_map_dump(struct pxrealm_map_t *m) {
        printk("%lu: offset %lu nrealms %d inuse %x private %p origin vol %llu "
               "size %llu\n",
               m->id, m->off, m->nrealms, m->inuse, m->private, m->volume_id,
               m->origin_size);
}
...
void pxrealm_debug_dump(void) {
        ...
        printk("global realm: cachedev %s\n\tcdev_size %llu\n\tcdev_sectors "
               ...
               pxrealm.map_indices, pxrealm.initialized, pxrealm.cdev);
...
static struct pxrealm_map_t *pxrealm_map(pxrealm_index_t id) {
...
static int compute_needed_realms(uint64_t size, pxrealm_hint_t hint) {
        switch (hint) {
        case PXREALM_LARGE:
...
static int pxrealm_assign(struct pxrealm_t *r, struct pxrealm_map_t *map,
                          int nrealms) {
        unsigned long v;

        v = bitmap_find_next_zero_area_off(r->realm_inuse_bitmap, r->max_realms,
                                           0, nrealms, 0, 0);
        if (v >= r->max_realms) {
                return -EBUSY;
        }
...
static int pxrealm_dealloc(struct pxrealm_map_t *pmap) {
        if (!pmap || !pmap->inuse) {
                return -EINVAL;
        }
...
pxrealm_index_t pxrealm_alloc(uint64_t volume_id, uint64_t origin_size,
        ...
        printk("%s volume %llu, origin size %llu, hint %d, context %p, nrealms "
               "%d\n",
               __func__, volume_id, origin_size, hint, context, nrealms);
...
int pxrealm_init(const char *cdevpath) {
        ...
        pxrealm.realm_inuse_bitmap =
            bitmap_zalloc(pxrealm.max_realms, GFP_KERNEL);
