...
static void pxrealm_dump_property(struct pxrealm_properties *prop) {
  printk(
      "realm %lu[%p]: offset %lu end %lu nsectors %lu hint %d origin "
      "vol %llu size %llu private %p\n",
      prop->id, prop->cdev, prop->offset, prop->end, prop->nsectors, prop->hint,
      prop->volume_id, prop->origin_size, prop->context);
}
...
struct pxmgr_context *pxmgr_cache_alloc(
    uint64_t vol_id, uint64_t vol_size, pxrealm_hint_t cache_hint, int cmode,
  ...
  cc = pxcc_init(mc->prop.cdev, mc->prop.offset, mc->prop.nsectors,
                 cache_blksize, vol_size, cmode, enqueue_to_origin, priv);
