...
/*
 * Hooks called from dev.c. Keep uring-specific work out of dev.c.
 *
 * pxd_uring_init_cb(): generic queue-cb init (in dev.c:fuse_queue_init_cb)
 *	calls into here for any uring-specific per-CB setup. Currently a
 *	no-op; this is the boundary so that future uring fields do not
 *	leak back into dev.c.
 *
 * pxd_uring_init_conn(): allocate fc->queue (struct fuse_conn_queues) and
 *	initialize its CBs via fuse_queue_init_cb (which is now also called
 *	from here). Called from dev.c:fuse_conn_init under the uring guard.
 *
 * pxd_uring_free_conn(): mirror cleanup, called from dev.c:fuse_conn_free_allocs.
 */
void pxd_uring_init_cb(struct fuse_queue_cb *cb)
{
	(void)cb;
}

int pxd_uring_init_conn(struct fuse_conn *fc)
{
	size_t sz = (sizeof(struct fuse_conn_queues) + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);

	fc->queue = vmalloc(sz);
	if (!fc->queue) {
		printk(KERN_ERR "pxd: failed to allocate uring queue (%zu bytes)\n", sz);
		return -ENOMEM;
	}
	memset(fc->queue, 0, sz);

	fuse_queue_init_cb(&fc->queue->requests_cb);
	fuse_queue_init_cb(&fc->queue->user_requests_cb);

	return 0;
}

void pxd_uring_free_conn(struct fuse_conn *fc)
{
	if (fc->queue) {
		vfree(fc->queue);
		fc->queue = NULL;
