...
void fuse_request_send_nowait(struct fuse_conn *fc, struct fuse_req *req)
{
	...
	trace_pxd_request(req->pxd_dev->dev_id, req->in.h.unique, req->pxd_rdwr_in.size, req->pxd_rdwr_in.offset, req->pxd_dev->minor, req_op(req->rq), req->rq->cmd_flags, req->in.h.opcode, req->pxd_rdwr_in.flags);
